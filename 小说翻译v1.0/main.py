...
class Application:
    ...
    def _execute_translation(self, source_file, target_lang, style, values, file_hash, worker):
        """执行分块翻译"""
        text = extract_text_from_docx(source_file) if source_file.endswith('.docx') else open(source_file).read()
    ...
    def _save_result(self, values, chunks, output_path):
        """保存翻译结果"""
        if values['-WORD-']:
            save_as_word(chunks, output_path)
        else:
