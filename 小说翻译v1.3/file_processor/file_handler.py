...
def dynamic_split(text, target_lang, max_tokens=FILE_HANDLER_CONFIG["CHUNKING"]["DEFAULT_MAX_TOKENS"]):
    ...
    :return: 分块后的文本列表，每个块内的段落之间以换行符分隔
    """
...
def extract_text_from_docx(docx_path):
    ...
    实现特性：
    - 保留原文分段（每个段落作为列表的一个元素，不进行额外的行拆分）
    - 如果段落中包含换行符，则原样保留（即内部仍可能包含'\n'，由save_as_word根据需要处理）
    - 过滤空段落

    :param docx_path: DOCX文件路径
    :return: 段落列表，每个元素代表一个原始段落
    :raises Exception: 文件解析失败时抛出
    """
    ...
    try:
        ...
        for para in doc.paragraphs:
            text = para.text  # 直接获取段落文本
            if text.strip():  # 过滤掉空段落
                paragraphs.append(text)
        logger.debug(f"[DOCX解析] 成功解析 | 段落数: {len(paragraphs)}")
        return paragraphs
    except Exception as e:
...
def extract_text_from_txt(txt_path):
    ...
    try:
        ...
        logger.debug(f"[TXT解析] 分割后段落数: {len(paragraphs)}")
        return paragraphs
