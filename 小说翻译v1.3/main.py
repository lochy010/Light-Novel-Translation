...
class Application:
    ...
    def process_document(self, values, worker):
        ...
        try:
            ...
            # 文件解析，获取原始段落列表
            original_paragraphs = self._process_file(source_file, worker)
            ...
            translated_chunks = self._execute_translation(
                source_file, target_lang, style, values, file_hash, worker, original_paragraphs
            )

            # 保存结果，传递原始段落列表以保留分段结构
    ...
    def _execute_translation(self, source_file, target_lang, style, values, file_hash, worker, original_paragraphs):
        """执行分块翻译，保留原始段落结构"""
        ...
        for orig_para in original_paragraphs:
            if orig_para.strip() and chunk_index < len(translated_chunks):
                # 有内容的原始段落对应一个翻译块
                final_translated_paragraphs.append(translated_chunks[chunk_index])
                chunk_index += 1
            else:
                # 空段落或超出翻译块数量时填充空字符串
                final_translated_paragraphs.append("")

        return final_translated_paragraphs
    ...
    def _save_result(self, values, translated_paragraphs,output_path):
        ...
        try:
            ...
            self.logger.debug(f"[Main] 保存成功 | 路径: {output_path}")
        except Exception as e:
            self.logger.error("[Main] 文件保存失败", exc_info=True)
            raise
