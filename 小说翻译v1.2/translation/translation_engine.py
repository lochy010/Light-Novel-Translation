...
class TranslationEngine:
    ...
    def _get_prompt_template(self, target_lang, style):
        ...
        if style == "custom":
            # 强制要求提示词必须包含翻译目标语言标记
            base_requirement = f"请严格使用{target_lang}进行翻译，并按照以下要求翻译，仅输出翻译后的文本：\n"

            if hasattr(self, 'gui') and (custom_prompt := self.gui.prompt_input.text().strip()):
                # 自动补全格式要求
                if "翻译后的文本" not in custom_prompt:
                    custom_prompt = f"{base_requirement}{custom_prompt}"
                logger.info(f"[提示词优化] 生效的自定义提示词\n{custom_prompt}")
                return custom_prompt
            else:
                # 强制的默认模板
                default_prompt = f"{base_requirement}{PROMPT_CONFIG['BASE_PROMPT'].get(target_lang, '')}"
                logger.warning("未检测到有效提示词，启用强制默认模板")
                return default_prompt
        else:
            # 强化预设模板的结构
            preset_prompt = (
                f"{PROMPT_CONFIG['BASE_PROMPT'].get(target_lang, '')}"
                f"{PROMPT_CONFIG['STYLE_PROMPT'].get(style, '')}"
                "\n请仅输出翻译后的文本"
            )
            logger.debug(f"[预设模板] 生效模板\n{preset_prompt}")
